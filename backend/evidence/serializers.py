...
class EvidenceSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if evidence_type == "VEHICLE":
            plate = (attrs.get("plate_number") if "plate_number" in attrs else getattr(self.instance, "plate_number", "")).strip()
            serial = (attrs.get("serial_number") if "serial_number" in attrs else getattr(self.instance, "serial_number", "")).strip()
            if plate and serial:
                raise serializers.ValidationError({"vehicle": "Vehicle evidence cannot have both plate_number and serial_number."})
            if not plate and not serial:
                raise serializers.ValidationError({"vehicle": "Vehicle evidence must have either plate_number or serial_number."})

        if evidence_type == "MEDICAL":
            single = (attrs.get("image_url") if "image_url" in attrs else getattr(self.instance, "image_url", "")).strip()
    ...
    def create(self, validated_data):
        # sync medical single<->list for convenience
        if validated_data.get("evidence_type") == "MEDICAL":
    ...
    def update(self, instance, validated_data):
        if (validated_data.get("evidence_type", instance.evidence_type)) == "MEDICAL":
