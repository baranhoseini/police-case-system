...
## Baran
- Established the backend project structure and core DRF configuration (apps/modules layout, environment setup).
- Implemented JWT-based authentication and configured API documentation (Swagger/OpenAPI).
- Built the RBAC foundation and role management capabilities to support dynamic role updates.
- Developed core backend modules (case/evidence baseline + suspects/rewards/stats foundations) and supported Docker/Postgres integration.
- Created and expanded backend spec/contract tests to validate CP1 requirements and protect key business flows.
- Led API–frontend contract alignment and stabilization work so UI integration remained smooth during CP2.
...
## Melina
- Implemented the frontend pages required by the specification (Home/Auth/Dashboard/Cases/Reports/Evidence/MostWanted/DetectiveBoard).
- Integrated the UI with real backend APIs (auth, stats, cases, evidence) and replaced mock flows with production-like data flow.
- Built user-facing capabilities like evidence upload and detective board export-to-image, and ensured the UI matched role-based rules.
- Improved UI/navigation and contributed to the test readiness of the frontend (unit + integration), supporting a reliable CP2 delivery.
